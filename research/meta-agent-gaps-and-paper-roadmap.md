...
|---|---|---|---|
| [MetaSyn](https://arxiv.org/abs/2606.17041) / [代码](https://github.com/BFTree/MetaSyn) | 442 个 Nature Portfolio source reviews、约 14 万条 PubMed corpus；把 retrieval、selection、synthesis 分开评价；公开配置、结果和测试 | 即使 retrieval Recall@200 达 90.9%，任何 pipeline 的 ground-truth included-literature recall 仍不超过 52.7%；主要是检索与 PI/ECO 纳排 benchmark，不覆盖真实双人提取、RoB、统计复算和 GRADE | 采用 stage-attributed metrics；优先攻克 eligibility 判定和 hard negatives，而不是只改善搜索排序 |
