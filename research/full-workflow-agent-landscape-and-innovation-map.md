...
8. 用真实人工工作流验证“节省时间且不牺牲关键召回”，而不只报告模型准确率。

因此，MetaWingman 最有价值的方向不是再增加几个会互相讨论的 agent，而是建设一个 **protocol-bound evidence adjudication system**：AI 负责定位、分解、反证和整理，人负责承担不可逆判断，规则与统计工具负责确定性执行，所有环节都有可回放的 provenance。
...
| MetaSyn | 442 个 Nature Portfolio meta-analyses、约 14 万 PubMed corpus；把 retrieval、screening、synthesis 分阶段评价 | retrieval ceiling Recall@200 为 90.9%，但任何 pipeline 的 included-literature recall 不超过 52.7%；显示 hard negatives 才是检索之后的约束 |
...
成熟度定义：A 可确定性自动化；B 适合 AI 辅助并抽样/双人核验；C 必须结构化人工判断；D 当前证据不足或需高强度专家仲裁。
...
| 全文筛选 | 长文本问答、理由生成 | 信息分散、否定证据、补充材料、翻译、排除理由首个/主因规则 | B/C | human-first/AI-second 可切换；全文页码锚点；冲突/不明进入仲裁 |
| 文献获取 | OA/API 下载、批量上传 | 许可、机构访问、机器人限制、补充材料和版本身份 | A/B | lawful resolver、license/checksum；账号操作隔离；失败原因可追踪 |
| 报告-研究谱系 | 相似度/注册号聚类 | companion、follow-up、重复 cohort、共享样本、多个 registry IDs | C | `record-report-study-result` graph；候选边+依据；人确认实体合并 |
| 数据提取 | schema extraction、source snippets | arm/outcome/timepoint 对齐；表/图/脚注；derived values；单位与方向 | B/C | 先提取 reported primitives；cell/region anchors；双盲提取；代码复算派生值 |
| RoB | 回答 signaling questions、给 justification | RoB 2 是 result-level；缺失信息、目标效应、多个报告和审评阈值 | C/D | AI 只定位证据/反证和建议回答；算法给 proposed judgement；人签署/说明 override |
...
创新点：**evidence-before-advice UI**。高风险节点默认 human-first：先记录人的初判和依据，再揭示 AI 建议与分歧；系统要求处理 strongest counterevidence 才能签署。低风险机械任务可 AI-first，以节省时间。
...
| selective prediction / abstention | 高风险系统应优化 risk-coverage，而非强迫每项作答 | 每个 criterion/field/domain 可单独弃权；预设最大可接受漏排/unsupported-value 风险和人工预算 |
| self-correction limits | 无外部反馈的自我纠错可能退化；同模型 checker 共享失败模式 | verifier 必须得到不同证据视图或确定性工具；不把多角色聊天当独立验证 |
| cognitive forcing | 人先作答、延迟显示建议、明确排除替代解释可减少 overreliance | RoB/GRADE/poolability 使用 blind human-first；记录建议前后变化，测 automation bias |
...
### 创新 4：Human-first high-stakes UX

对 RoB、GRADE、poolability 和结论，先收集人的盲法初判；随后才显示 AI。对题录初筛和格式检查可 AI-first。把人机顺序作为可配置、可随机化、可评价的干预，而不是界面细节。
...
4. **Prospective workflow layer**：真实 reviewer 使用实验，测时间、关键错误、过度依赖、仲裁负担和复现性。
...
- manual minutes saved at prespecified safety floor；
- risk-coverage/abstention curve；
- rerun agreement、provider/model drift、cost、latency、failure/retry rate；
- human initial judgement、AI exposure 后改变率、正确/错误 advice 下 overreliance。
...
- fixed automation vs risk-adaptive oversight。
...
6. human-first/AI-first dual-review UI data model；
...
先做 RoB 2 signaling-question dossier，再扩展 ROBINS-I/QUADAS-2；随后做 estimand alignment 和 poolability；最后做 GRADE domain dossier。每一步都将“找证据”“规则建议”“最终 judgment”分离。

### P3：验证完整闭环

选择少量可公开复现、材料齐全的 reviews，完成从协议到 claim ledger 的端到端重建。只有在 component 和 stage safety floors 通过后，才评价整条流水线的节时。
...
- 继续扩充统计图形数量；
- 声称无人全自动；
- 在没有 ground truth 和人工时长的情况下做漂亮 demo。

## 八、一句话差异化

> MetaWingman 不替研究者“自动做出”系统综述判断；它把协议、证据、人工分歧、确定性计算和结论连接为一条可检查、可弃权、可复算、可更新的决策链。
...
- Xie A, et al. [Benchmarking LLM Agents on Meta-Analysis Articles from Nature Portfolio](https://arxiv.org/abs/2606.17041). preprint, 2026.
...
- Holst M, et al. [PRISMA-trAIce](https://ai.jmir.org/2025/1/e80247/). *JMIR AI*, 2025.
