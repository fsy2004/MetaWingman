...
- [ ] VAL-1: Select and license-review the first published-review reconstruction families without contaminating the held-out split. Eighteen formally identified reviews are in the broad discovery catalog and four are in the strict registry. Five immutable material plans now cover BMJ living-NMA method code, the PLOS living-DTA pair, carbon-pricing screening, SCI extraction-to-analysis and HEPSANET controlled-IPD guards. **2026-08-18: first promotion drafted — sci-exercise-analysis (dev split, analysis slice; BSD-3-Clause complete, sealed input hashed); ag-rdt blocked on 2022 heiDATA inventory; carbon-pricing blocked on license/cutoff; bmj + hepsanet are guard tests by design** (`val1-promotion-analysis-2026-08-18.md`).
