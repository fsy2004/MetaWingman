...
> 北极星：AI 执行十阶段全流程的可逆、可验证、可审计主路径；人工只在预设的
> **审查复核门**介入——协议冻结、纳排终审、RoB 终判、GRADE 定级、结论签署、
> 账号授权、不可逆提交。每个阶段定义 AI 执行方法（信息学来源）、人工复核
> 门、现状与下一步。方法参照以信息学/CS 为主（编排、工具、验证、结构化
> 生成、成本、评估），医学领域方法只在窄任务组件处引用。

## 1. 阶段 × AI 执行方法 × 人工复核门

| 阶段 | AI 执行方法（信息学来源） | 人工复核门 | 现状 | 下一步 |
|---|---|---|---|---|
