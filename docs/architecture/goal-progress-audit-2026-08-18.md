...
1. **GLM 跨模型实证整合**（子代理运行中，服务器 2h48m，sr 超 800 运行）→ 白皮书"跨模型"行。
2. **VAL-2c 人工评审**（评分表已备好，待用户；kappa 决定 V4 路径）。
3. **ag-rdt 操作语料**（检索策略子代理在途 → 语料构建器实跑 → 阻塞③解除）。
4. **VAL-3 AI-only 端到端**（机制演练已完成；AI-only 配置 pilot 依赖更多家族材料）。
5. **TOPIC-1..4** 选题引擎扩展（图摄取/泄漏审计/基线对比/前瞻注册）——设计已具备，实施量大。
