...
1. **GLM 跨模型实证整合** → **✅ 已完成（Round 39）**：C3 同盲集对拍
   （F1 0.8816 vs 0.9385，999/999、0 弃权；检索选择准确率 0.96 vs 0.93）+
   provider kappa 0.8472（0.8221–0.8722），`glm-cross-provider-results-2026-08-18.md`。
2. **VAL-2c 人工评审** → **✅ 已评分（kappa 0.311）并触发转向**：规则与准则判断不符 → 修订规则或换准则监督标签；已决策：**准则监督重标注**（800 dev 已评 → 全量 7,932 train + 1,174 dev 评分进行中）→ rubric V2 训练 → 新代 VAL-2c 验证（`appraisal-task-relabeling-decision-2026-08-18.md`）。
...
4. **VAL-3 AI-only 端到端**（机制演练已完成；AI-only 配置 pilot 依赖更多
   ...
   **首个 AI 筛选 pilot 已跑（Round 41）**：黄金召回 0.765（114/149）、
   保留率 19.4%、弃权为主损失——`val3-ai-screening-pilot-results`。
