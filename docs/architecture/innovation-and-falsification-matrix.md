...
| I2 | uncertainty estimation, semantic entropy, conformal prediction, model routing | **Scientific risk-coverage controller:** calibrates task-specific asymmetric harms, evidence coverage, deterministic conflicts, rerun disagreement and provider diversity to choose compute, opposition or abstention | At a frozen false-exclusion or unsupported-value ceiling, the controller increases automated coverage versus fixed-call and confidence-threshold routing | one-call model; fixed proposer-judge; semantic entropy alone; full controller | risk ceiling violated on held-out families or coverage is not improved |
