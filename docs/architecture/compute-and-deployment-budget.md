...
The first live adapter uses DeepSeek's official OpenAI-compatible endpoint. On 2026-08-15, the account-visible model list and minimal structured requests were verified for `deepseek-v4-flash` and `deepseek-v4-pro`. A provider-neutral configuration and generic OpenAI-compatible adapter passed the same live probe, while a schema-gated abstention candidate completed in one attempt using 619 total tokens. This establishes connectivity and contract behavior, not task accuracy. Prices and model names are provider state, so each frozen benchmark must query and record live state rather than rely on this paragraph.
