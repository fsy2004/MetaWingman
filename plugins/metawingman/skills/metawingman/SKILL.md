---
name: metawingman
description: End-to-end, evidence-grounded systematic review and meta-analysis support for topic selection, protocol and registration, live database searching, lawful full-text retrieval, deduplication, dual screening, extraction, study/result lineage, risk-of-bias assessment, quantitative or SWiM synthesis, GRADE, manuscript writing, AI reviewer audit, revision, and living updates. Use whenever asked to plan, conduct, automate, analyze, write, audit, update, or peer-review a systematic review, scoping review, evidence synthesis, or meta-analysis in biomedicine or related fields. Supports pairwise, network, diagnostic, prognostic, prevalence, incidence, dose-response, IPD, Bayesian, qualitative, mixed-methods, and living review profiles. Requires live source verification, auditable provenance, reproducible code, and explicit human decisions; never invent references, screening decisions, extracted values, or completed analyses. 中文触发：系统综述、系统评价、荟萃分析、meta 分析、Meta分析、PRISMA、GRADE、证据合成、meta 论文。
---
...
## Skill-driven method contract

MetaWingman's method contribution is a strong evidence-synthesis agent, not safety alone. Integrity, legal access, sealing, and human responsibility are guardrails around the original four-part method core:

- **Review Question Certificate:** derive the review question from primitives, first-principle assumptions, mechanism or decision model, evidence tension, minimal decisive test, expected observations, failure-update rule, and novelty gate.
- **Socratic stage reflection:** before each stage, ask the method questions a strong reviewer would ask; after each stage, record failures, repairs, and lessons.
- **PRM-style step verification:** decompose screening, extraction, appraisal, synthesis, certainty, and claims into source- or tool-verifiable steps.
- **Meta-update and distillation:** convert verified failures and repairs into Skill, prompt, verifier, trajectory, and student-bootstrap improvements.

The paper-facing implementation compresses this core into two executable methods:

- **Decision-aware topic opportunity control**: derive review opportunities from a cutoff evidence landscape by identifying a downstream decision tension, the minimal decisive question, the required method or estimand guardrail, evidence-missingness anchors, disconfirmation checks, independent signal audit, portfolio diversity, gates, stopping rules, and frozen or prospective evaluation.
- **Conclusion-directed risk-impact evidence acquisition**: keep a typed residual-risk state for material claims, then choose among lawful source, query, full-text, screening, verifier, and compute actions by residual risk, downstream claim impact, and asymmetric harm. Execute the selected action, verify artifacts, recompute the risk state, replan, and stop only under the accountable stopping rule.

At every stage, drive prompts and tool plans through this loop: form the certificate, name the decision tension, design the strongest disconfirmation, bind evidence gaps to source nodes, verify the conclusion-changing step, state the stopping rule, and write the meta-update. Agent training or distillation may stabilize this behavior; it should make the agent bolder and more methodologically creative while keeping claims evidence-bound.
...
- For topic discovery, build a time-bounded evidence landscape and apply **decision-aware topic opportunity control**. The host agent may propose frameworks, evidence-node references, interpretations, decision tension, minimal decisive question, method or estimand guardrail, evidence-missingness anchors, portfolio cluster, and disconfirmation searches, but it must not assign its own opportunity scores. Validate proposal batches against `topic_proposal_batch.schema.json`; independently audit overlap, feasibility, decision relevance, uncertainty, evidence maturity, nonduplication, update need, equity, cross-domain value, contamination, and ambiguity before creating `topic_candidate` records. Then run `python scripts/select_topics.py <landscape.json> <candidates.jsonl>` under frozen weights and gates. Never convert journal prestige, citation volume, or a fluent rationale into ground truth.
- Jointly design the clinical question and synthesis route with the closed `clinical_decision_context`, `question_synthesis_candidate`, `method_route_decision`, and `question_synthesis_search` contracts. Deterministic compatibility checks and source/executable verifiers control selection; proposer, opposition, and judge calls from one provider are test-time compute, not independent scientific evidence. Preserve SWiM and `no_pooling` as valid outcomes.
...
- Treat search and verification as **conclusion-directed risk-impact evidence acquisition**. Material acquisition states belong in `02_search/acquisition/evidence_acquisition_states.jsonl`; run `python scripts/plan_evidence_acquisition.py <state.json>` to rank only lawful, credential-compatible source, query, full-text, screening, verifier, and compute actions by residual risk, downstream claim impact, and asymmetric harm. Execute selected actions through `scripts/run_evidence_acquisition_loop.py`, verify artifacts, recompute the risk state, and replan before any stop. A `stop_candidate` still requires the accountable stopping review; an uncalibrated or inaccessible high-impact gap must abstain.
