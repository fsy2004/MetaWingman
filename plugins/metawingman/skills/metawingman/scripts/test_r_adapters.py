...
def main() -> int:
    ...
    for manifest_path in sorted(manifests.glob("*.json")):
        ...
        output = outdir / mid; output.mkdir(parents=True, exist_ok=True)
        ...
        try:
            proc = subprocess.run(cmd, cwd=output, capture_output=True, text=True, encoding="utf-8", errors="replace", timeout=args.timeout)
    ...
    (outdir / "adapter-regression.json").write_text(json.dumps(summary, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
