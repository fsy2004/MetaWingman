#!/usr/bin/env python3
"""Score locked acquisition outputs against a sealed published reference."""
...
from metawingman.scripts.metawingman_core.acquisition_scoring import (
    ScoringError, extract_reference_rows, match_reference_rows, score_rankings,
    validate_exact_receipt_slots, validate_scoring_gate,
)
...
def main() -> int:
    ...
    plan = json.loads(args.plan.read_text(encoding="utf-8")); lock = json.loads(args.lock.read_text(encoding="utf-8"))
    ...
    for receipt in receipts:
        ...
        outputs.append((receipt, json.loads(output_path.read_text(encoding="utf-8"))))
    ...
    reference_rows, extraction_audit = extract_reference_rows(reference)
    ...
    for receipt, output in outputs:
        scores.append({"configuration_id": receipt["configuration_id"], "seed": receipt["seed"], **score_rankings(output, matched, valid_corpus_ids=valid_ids), "input_tokens": receipt["input_tokens"], "output_tokens": receipt["output_tokens"], "wall_seconds": receipt["wall_seconds"]})
    result = {"schema_version": "1.0", "plan_id": plan["plan_id"], "reference_sha256": sha256(reference), "extraction_audit": extraction_audit, "mapping_audit": mapping_audit, "scores": sorted(scores, key=lambda row: (row["configuration_id"], row["seed"]))}
    atomic_write_json(args.out, result); print(json.dumps(result, indent=2))
    return 0
