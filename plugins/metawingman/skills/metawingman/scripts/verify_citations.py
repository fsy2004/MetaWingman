...
def main() -> int:
    ...
    email = (os.getenv("CROSSREF_EMAIL", "") or os.getenv("NCBI_EMAIL", "")).strip()
    ua = "MetaWingman/1.0" + (f" mailto:{email}" if email else "")
    ...
    for row in rows:
        ...
        try:
            if doi:
                ...
                if email: url += "?mailto=" + urllib.parse.quote(email)
                ...
                params = {"db": "pubmed", "id": pmid, "retmode": "xml", "tool": "systematic_review_meta_analysis"}
