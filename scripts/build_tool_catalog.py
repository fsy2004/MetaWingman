...
def main() -> int:
    ...
    toolkit = root / "scripts/r/toolkit/R"; adapters = root / "scripts/r/adapters"; manifests = root / "scripts/r/manifests"
    ...
    for path in sorted(toolkit.glob("*.R")):
        ...
        lines.append(f"| `scripts/r/toolkit/R/{path.name}` | {', '.join(f'`{x}`' for x in funcs) or 'internal helpers'} |")
